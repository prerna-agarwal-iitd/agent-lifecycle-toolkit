...
logger = logging.getLogger(
    "toolops.validation.test_case_generation_utils.combination_modules.all_param_testcase"
)


# This method generates one testcase with all paramteres #
def all_param_testcase(
    ...
    if (
        len(
            set(
                transformed_tool_spec_postprocess["operation"][operation_type].keys()
            ).intersection({"request_body_params", "path_params", "query_params"})
        )
        != 0
    ):
        ...
        if (
            len(
                transformed_tool_spec_postprocess["operation"][operation_type][
                    "request_body_params"
                ].keys()
            )
            > 0
    ...
    while number_of_tries <= 1:
        ...
        while incorrectly_generated_data:
                ...
                if len(data_generated_through_LLM_so_far) > 0:
                    for case in data_generated_through_LLM_so_far:
                        for key in list(data_generated_through_LLM_so_far[case].keys()):
                            if (
                                data_generated_through_LLM_so_far[case][key] == ""
                                or data_generated_through_LLM_so_far[case][key] == " "
                            ):
                                correctly_generated_values = False
                                break
                else:
                    correctly_generated_values = False
                if correctly_generated_values:
                    ...
                    for case in data_generated_through_LLM_so_far:
                        for key in list(data_generated_through_LLM_so_far[case].keys()):
                            if (
                                key
                                not in list(
                                    data_generated_through_LLM["testcase_1"].keys()
                                )
                                and key in all_keys
                            ):
                                data_generated_through_LLM["testcase_1"][key] = (
                                    data_generated_through_LLM_so_far[case][key]
                                )
                        break
            except Exception:
            ...
            if count_failed_execution == 2:
                break
    logger.info(
        "One testcase generated with all parameters",
        extra={"details": data_generated_through_LLM},
    )
