...
logger = logging.getLogger(
    "toolops.generation.test_case_generation.test_case_generation_utils.testcases_from_optionally_provided_data"
)


# This method generates testcases based on optional data provided by the user #
def generate_testcases_optional_data(
    ...
    num_testcases_to_generate_per_scenario_except_last = int(
        max_number_testcases_to_generate / len(optional_data_scenario)
    )
    num_testcases_to_generate_per_scenario_last = (
        max_number_testcases_to_generate
        - num_testcases_to_generate_per_scenario_except_last
        * (len(optional_data_scenario) - 1)
    )
    count_testcases = 0
    ...
    for scenario_index in range(len(optional_data_scenario)):
        if scenario_index < len(optional_data_scenario) - 1:
            testcases_for_current_scenario = (
                num_testcases_to_generate_per_scenario_except_last
            )
        else:
        ...
        while incorrectly_generated_data:
            try:
                prompt = (
                    """You are an expert in generaing testcases with realistic values without the word 'example_' for a combination of mandatory and optional parameters present in a spec. Currently I have an incomplete testcase such as\n"""
                    + json.dumps(optional_data_scenario[scenario_index])
                    + """\n For each testcase use the values of the all the parameters given in the incomplete testcase. In each of the testcase you never exclude any mandatory parameters. In each testcase you never exclude any optional parameter values that are given in the incomplete testcase. Do not generate any new values for the parameters given in the incomplete testcase. Generate """
                    + str(testcases_for_current_scenario)
                ...
                data_generated_through_LLM_so_far = post_process_testcase(
                    data_using_LLM(prompt, client, gen_mode)
                )
                incorrectly_generated_data = False
                for case in data_generated_through_LLM_so_far:
                    count_testcases = count_testcases + 1
                    data_generated_through_LLM["testcase_" + str(count_testcases)] = (
                        data_generated_through_LLM_so_far[case]
                    )
                    data_generated_through_LLM_from_optional_data[
                        "testcase_" + str(count_testcases)
                    ] = data_generated_through_LLM_so_far[case]
                    if count_testcases == max_number_testcases_to_generate:
                        correct_number_of_testcases_generated = True
                        break
                if correct_number_of_testcases_generated:
                    break
            except Exception:
                ...
                incorrectly_generated_data = True
    logger.info(
        "Testcases generated from optional data provided",
        extra={"details": data_generated_through_LLM_from_optional_data},
    )
